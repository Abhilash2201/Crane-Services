...
const initialStatuses = Object.fromEntries(SUITE.flatMap((s) => s.tests.map((t) => [t.id, "pending"]))) as Record<string, TestStatus>;

export default function App() {
  const [statuses, setStatuses] = React.useState<Record<string, TestStatus>>(initialStatuses);
  ...
  const [logs, setLogs] = React.useState<LogEntry[]>([{ t: "00.0", type: "info", message: "Waiting to run" }]);
  const [failedAudit, setFailedAudit] = React.useState<AuditEntry[]>([]);

  const stateRef = React.useRef<QaState>({ tokens: {}, data: {} });
  const startRef = React.useRef<number>(Date.now());
  const api = React.useMemo(() => createApi(BASE, (role) => stateRef.current.tokens[role]), []);
  ...
  const runAll = React.useCallback(async () => {
    ...
    for (let i = 0; i < all.length; i += 1) {
      ...
      if (!fn) {
        ...
        setDetails((prev) => ({ ...prev, [test.id]: `Missing runner: ${test.fnKey}` }));
      } else {
        try {
          const res = await fn();
          ...
          addLog(`[${res.status.toUpperCase()}] ${test.name} - ${res.detail}`, res.status === "pass" ? "pass" : res.status === "fail" ? "fail" : res.status === "warn" ? "warn" : "info");
        } catch (error) {
  ...
  return (
    <div className="container">
        ...
        <div className="header-inner">
          <div className="logo-row">
            ...
            <div>
              ...
              <div className="subtitle">END-TO-END TEST REPORT | GENERATED {now}</div>
            </div>
          </div>
      ...
      <div className="notice">
        Live Server/local domains must be allowed in backend CORS_ORIGINS. Run from localhost dev server, not file://.
      </div>

      <Controls running={running} progressPct={progressPct} progressLabel={progressLabel} onRun={runAll} />

      <div className="section-title">App Targets</div>
      <table className="cred-table">
        <thead><tr><th>APP</th><th>URL</th></tr></thead>
        <tbody>
          <tr><td>Customer</td><td>{APP_URLS.customer}</td></tr>
          <tr><td>Owner</td><td>{APP_URLS.owner}</td></tr>
          <tr><td>Driver</td><td>{APP_URLS.driver}</td></tr>
          <tr><td>Admin</td><td>{APP_URLS.admin}</td></tr>
        </tbody>
      </table>

      {SUITE.map((s) => <TestSection key={s.id} suite={s} statuses={statuses} details={details} times={times} />)}
