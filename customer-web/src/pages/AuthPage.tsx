...
export function AuthPage() {
  ...
  const handleSendOtp = () => {
    if (!isPhoneValid) { setError("Enter a valid 10-digit Indian mobile number."); setInfo(""); return; }
    const nextOtp = String(Math.floor(100000 + Math.random() * 900000));
    setGeneratedOtp(nextOtp); setError(""); setInfo(`OTP sent to +91 ${phone}. Demo OTP: ${nextOtp}`); setStep("otp");
  };

  const handleOtpChange = (value: string, index: number) => {
    const clean = value.replace(/\D/g, "").slice(0, 1);
    setOtp((prev) => { const next = [...prev]; next[index] = clean; return next; });
  };

  const handleVerify = () => {
    if (otp.join("").length !== 6) { setError("Please enter all 6 OTP digits."); return; }
    if (otp.join("") !== generatedOtp) { setError("Invalid OTP. Please try again."); return; }
    setError(""); setInfo("Verification successful. Redirecting..."); setTimeout(() => navigate("/dashboard"), 700);
  };
        ...
        <CardContent style={{ display: "grid", gap: 12 }}>
          {step === "phone" ? (
            <>
              ...
              <Input placeholder="98XXXXXX45" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, "").slice(0, 10))} />
            ...
            <>
              ...
              <OtpRow>{Array.from({ length: 6 }).map((_, i) => <Input key={i} inputMode="numeric" maxLength={1} value={otp[i]} onChange={(e) => handleOtpChange(e.target.value, i)} style={{ textAlign: "center" }} />)}</OtpRow>
